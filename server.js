...
// ══════════════════════════════════════════════════════════════
const SYSTEM_PROMPT = `You are CA AI PY SOSA DHRUV — an elite AI Chartered Accountant with 25+ years of experience, registered as FCA with ICAI. You are a SPECIALIST in Indian taxation, GST, corporate law, auditing, financial planning, and business advisory. Respond EXACTLY like a senior CA sitting across the table from a client.
...
function generatePDF(messages, clientName, title) {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(20).text('CA AI PY SOSA DHRUV', 50, 50);
    ...
    messages.forEach((msg, i) => {
      ...
      const label = isUser ? '👤 Client' : '🤖 CA AI PY SOSA DHRUV (FCA)';
    ...
    doc.fillColor(GREY).font('Helvetica').fontSize(9).moveDown(6)
      .text('This report is generated by CA AI PY SOSA DHRUV, an AI tool trained on Indian tax laws and accounting standards (AY 2025-26). The content is for professional guidance only and should be verified against the latest government notifications before official filing.', { width: W, align: 'justify', lineGap: 2 });
    doc.moveDown(1);
    doc.fillColor(INDIGO).font('Helvetica-Bold').fontSize(10).text('CA AI PY SOSA DHRUV — AI Chartered Accountant | FCA ICAI Knowledge Base | India | AY 2025-26');
    doc.end();
  });
}
...
function generateExcel(messages, clientName, title) {
  ...
  const convRows = [
    ['CA AI PY SOSA DHRUV — AI Chartered Accountant Report'],
  ...
  messages.forEach((m, i) => {
    convRows.push([i + 1, m.role === 'user' ? 'Client' : 'CA AI PY SOSA DHRUV (FCA)', (m.content || '').replace(/\*\*?/g, '').replace(/#+\s/g, '').slice(0, 3000)]);
  });
...
app.post('/api/export/pdf', async (req, res) => {
  ...
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="CA_AI_PY_SOSA_DHRUV_Report_${Date.now()}.pdf"`);
    res.send(buf);
  } catch (err) {
...
app.post('/api/export/excel', (req, res) => {
  ...
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="CA_AI_PY_SOSA_DHRUV_Report_${Date.now()}.xlsx"`);
    res.send(buf);
  } catch (err) {
...
server.listen(PORT, () => {
  console.log('\n╔══════════════════════════════════════════════════╗');
  console.log('║   CA AI PY SOSA DHRUV — AI CA v4.0              ║');
